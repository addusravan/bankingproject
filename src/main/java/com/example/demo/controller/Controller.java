...
accNo1,@PathVariable String accNo2,@PathVariable int amount) {
        ...
        balance_repo.findAll().filter(user -> user.getAccNo().equals(accNo1) && user.getBalance() >= amount)
                .doOnNext(ab -> {
                    ab.setBalance(ab.getBalance() - amount);
   ...
   Balance B1=balance_repo.findByAccNo(accNo1);
        balance_repo.findAll().filter(user2 -> user2.getAccNo().equals(accNo2) && B1.getBalance()>=amount )
                .doOnNext(ab -> {
                    ab.setBalance(ab.getBalance() + amount);
                })
                .flatMap(balance_repo::save)
